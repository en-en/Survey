    ...
    public class ProjectService : BaseService, IProjectService
    {
        ...
        public async Task ReportSurveyDataAsync(ReportSurveyDataRequest request)
        {
            using (var lk = _redisClient.Lock(string.Format(CygConstant.ProjectLockKey, request.ProjectId), 5))
            {
                ...
                #endregion
                //获取杆型方案
                var towers = new List<TowerRequest>();
        ...
        /// <returns></returns>
        [UseTransaction]
        public async Task RepairData(List<string> request,bool isCoverDesign)
        {
            ...
            foreach (var projectId in request)
            {
                ...
                try
                {
                   ...
                   // var surveyTowerExts = await DbHelper.FindAllAsync<SurveyTowerExt>(x => x.ProjectId == projectId);
                    ...
                    if (isCoverDesign) requestInfo.DesignData.Towers = requestInfo.SurveyData.Towers;
                        #endregion
                    ...
                    if (isCoverDesign) requestInfo.DesignData.Cables = requestInfo.SurveyData.Cables;
                    #endregion
                    ...
                    if (isCoverDesign) requestInfo.DesignData.CableChannels = requestInfo.SurveyData.CableChannels;
                    #endregion
                    ...
                    if (isCoverDesign) requestInfo.DesignData.CableEquipments = requestInfo.SurveyData.CableEquipments;
                    #endregion
                    ...
                    (from item in designOverheadEquipments
                     select
                            new OverheadEquipmentRequest
                            {
                                ...
                                Sub_ID = item.Sub_ID,
                                Capacity = item.Capacity==null? default(TransformerCapacity): ("kVA" + item.Capacity.Replace("kVA", "")).ToEnum<TransformerCapacity>(),
                    ...
                    if (isCoverDesign) requestInfo.DesignData.OverheadEquipments = requestInfo.SurveyData.OverheadEquipments;
                    ...
                    if (isCoverDesign) requestInfo.DesignData.Lines = requestInfo.SurveyData.Lines;
        ...
        public async Task ReportDisclosureDataAsync(ReportDisclosureDataRequest request)
        {
            using (var lk = _redisClient.Lock(string.Format(CygConstant.ProjectLockKey, request.ProjectId), 5))
            {
                ...
                #region 写入轨迹数据
                await DbHelper.DeleteAsync<ProjectTrackRecord>(it => it.ProjectId == request.ProjectId && it.RecordType == ProjectTraceRecordType.交底);
                if (request.TrackRecords.HasVal())
                {
                    ...
                    {
                        it.ProjectId = request.ProjectId;
                        it.RecordType = ProjectTraceRecordType.交底;
                    });
                    await DbHelper.BulkInsertAsync(tracks);
                }
                #endregion
            ...
            if (_overheadEquipments.HasVal())
            {
                foreach (var overheadEquipment in _overheadEquipments)
                {
                    if (overheadEquipment.Type == OverheadEquipmentType.变压器)
                    {
                        var curCapacity = (TransformerCapacity)(overheadEquipments
                            .FirstOrDefault((x => x.Id == overheadEquipment.Id)).Capacity);
                        overheadEquipment.Capacity = curCapacity.GetText();
                        //获取变压器主杆
                        var mainTower = towers.Find(x => x.Id == overheadEquipment.Main_ID);
                        //获取变压器副杆
                        var subTower = towers.Find(x => x.Id == overheadEquipment.Sub_ID);
        ...
        /// <returns></returns>
        private async Task<List<BatchTowerViewResponse>> TowerModules(List<TowerRequest> towers, string resourceLibID)
        {
                    ...
                    foreach (var item in towers)
                    {
                        ...
                        request.BatchModuleQueries.Add(
                            new BatchModuleQueryRequest
                            {
                                ...
                                SegmentMode = item.T_Segment.ToString()=="无"?"":item.T_Segment.ToString(),
                                Height = Convert.ToDouble(height),
                                Arrangement = item.T_Sort.ToString()=="无"?"":item.T_Sort.ToString(),
        ...
        private int GetSymbolId(object obj,object sourceObj)
        {
            ...
            try
            {
                if (obj.GetType() == typeof(TowerRequest))
                {
                    var curData = (TowerRequest)obj;
        ...
        /// </summary>
        /// <returns></returns>
        private async Task<List<TransformerResponse>> GetTransformerList(List<OverheadEquipmentRequest> requests, List<TowerRequest> towerRequests, string resourceLibId)
        {
