...
namespace Cyg.Applicatio.Repository.Impl
{
    public class ProjectRepository : BaseRepository, IProjectRepository
    {
        ...
        public async Task<ProjectDetailResponse> GetDetailAsync(string projectId)
        {
            //      var query = new SqlBuilder();
            //      query.AddSql(@"
            //      SELECT ptp.Id,ptp.ProjectName,ptp.ParentId ProjectCollectionId,
            //      ptpc.ProjectName ProjectCollectionName,ptpc.ProjectType,
            //      ptpc.ProjectCode,ptpc.KVLevel,ptpc.Construction,ptpc.Stage,ptpc.Batch,ptpc.Province,
            //      ptpc.City,ptpc.Area,ptpc.Meteorologic,ptpc.CompanyId,ptp.CreatedOn,ptps.FlowState,
            //      ptps.IsResetSurvey,ptp.Remark,ptpc.BatchYear,ptpc.PileRange,ptpc.DisclosureRange
            //      FROM pdd_t_project ptp
            //   INNER JOIN pdd_t_projectcollection ptpc ON ptp.ParentId=ptpc.ID
            //      INNER JOIN pdd_t_users ptu ON ptp.CreatedBy=ptu.ID
            //INNER JOIN pdd_t_projectstate ptps ON ptps.ProjectID=ptp.ID
            //      WHERE ptp.ID=@ProjectId AND (ptp.CreatedBy=@UserId OR  
            //      EXISTS(SELECT 0 FROM pdd_t_projectauthority AS ptpa 
            //      WHERE ptpa.ProjectID=ptp.ID AND ptpa.UserID=@UserId AND ptpa.AuthorityCategory=@AuthorityCategory));");
            //      query.AddParam("ProjectId", projectId);
            //      query.AddParam("UserId", CurrentUser.Id);
            //      query.AddParam("AuthorityCategory", ProjectAuthorityCategory.Survey);
            var query = new SqlBuilder();
            query.AddSql(@"
